...
- Frontend design system (since the 2026-09 redesign): light stone paper with navy ink, colour reserved for direction (`up` / `down` / `caution` tokens in `globals.css`), Newsreader for text and IBM Plex Mono for figures and tickers; the agent run panel (`RunPanel.tsx`) is the only dark surface.
