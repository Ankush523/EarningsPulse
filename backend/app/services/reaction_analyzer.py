...
class ReactionAnalyzer:
    ...
    def aggregate_events(
        ...
        *,
        ...
        historical_move_pct = round(mean(abs(e.initial_move_pct) for e in events), 2) if events else None

        implied_move_pct: float | None = None
        volatility_assessment: str | None = None
        options_summary: str | None = None

        if options_data and options_data.get("implied_move_pct") is not None:
        ...
        if implied_move_pct is not None and historical_move_pct is not None:
            ...
            options_summary = f"Options market implies ±{implied_move_pct:.1f}% move for upcoming earnings."
        elif historical_move_pct is not None:
            options_summary = f"Historical average realized move is ±{historical_move_pct:.1f}% across past earnings."
